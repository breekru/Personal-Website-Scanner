...
class WebsiteVerificationTool:
    ...
    def perform_website_checks(self, url):
        """Perform all website verification checks"""
        result = {
            'registrar': 'Unknown',
            'page_title': 'Unknown',
            'status_code': 0,
        ...
        try:
            ...
            # 2. HTTP request for page title and status
            try:
                headers = {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
                }
                response = requests.get(url, timeout=15, verify=False, headers=headers, allow_redirects=True)
                ...
                result['additional_checks']['http_error'] = str(e)
                ...
                try:
                    ...
                    with socket.create_connection((domain, 443), timeout=10) as sock:
                        with context.wrap_socket(sock, server_hostname=domain) as ssock:
                            ...
                            result['ssl_issuer'] = cert.get('issuer', [{}])[0].get('organizationName', 'Unknown')
                            result['ssl_expiry'] = cert.get('notAfter', 'Unknown')
                except Exception as e:
                    result['ssl_valid'] = False
                    result['additional_checks']['ssl_error'] = str(e)
